...
const Chatbot = () => {
  ...
  // TTS for latest bot message (robust — no mid-typing cutoffs, no repeats)
  useEffect(() => {
    if (typeof window === 'undefined' || !('speechSynthesis' in window)) return;

    const synth = window.speechSynthesis;

    // If speaker is off, stop and reset markers
    if (!speakerOn) {
      synth.cancel();
    ...
    const textToSpeak = (lastBot.text || '').trim();

    // Skip placeholders / empty
    if (!textToSpeak || textToSpeak === 'Thinking...') return;

    // If we're already speaking this text, do nothing
    if (speakingTextRef.current === textToSpeak && synth.speaking) return;

    // If we already finished speaking this exact text, do nothing
    ...
    try {
      ...
      utter.onend = () => {
        lastSpokenRef.current = textToSpeak; // mark completed
        speakingTextRef.current = null; // clear in-progress
        currentUtterRef.current = null;
      };
      utter.onerror = () => {
        speakingTextRef.current = null;
      ...
      speakingTextRef.current = textToSpeak;
      currentUtterRef.current = utter;
      synth.speak(utter);
    } catch {
      // fail silently — avoid breaking chat
    }
  }, [msgs, speakerOn]);

  // Stop speech only when component unmounts
  useEffect(() => {
    return () => {
